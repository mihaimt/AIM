...
void FV::apply_bc()
{
   ...
   else if(param.bc == open_atm)
   {
      for(unsigned int i = 0; i< ng_cell; ++i)
      {
          if(param.valve == LEFT || param.valve == ALL)
          {
              primitive[i][0] = primitive[ng_cell][0];
              primitive[i][1] = primitive[ng_cell][1];
              primitive[i][2] = param.atm_pressure;
          }
          else
          {
              primitive[i][0] = primitive[2*ng_cell - i - 1][0];
          ...
          if(param.valve == RIGHT || param.valve == ALL)
          {
              primitive[n_cell + ng_cell + i][0] = primitive[n_cell-1][0];
              primitive[n_cell + ng_cell + i][1] = primitive[n_cell-1][1];
              primitive[n_cell + ng_cell + i][2] = param.atm_pressure;
          }
          else
          {
              primitive[n_cell + ng_cell + i][0] = primitive[n_cell + ng_cell - i - 1][0];
...
void FV::run ()
{
   ...
   while (time < param.final_time && iter < param.max_iter)
   {
      ...
      {
         cout<<"WARNING: HIGH PRESSURE DETECTED!!!"<<endl;
         if(param.valve == NONE)
            cout<<"VALVES STILL CLOSED!!!"<<endl;
         else if(param.valve == LEFT)   
            cout<<"LEFT VALVE OPENED!!!"<<endl;
         else if(param.valve == RIGHT)   
            cout<<"RIGHT VALVE OPENED!!!"<<endl;
         else if(param.valve == ALL)   
            cout<<"ALL VALVES OPENED!!!"<<endl;      
         
         if(param.valve != NONE)
            if(param.bc != open_atm)
               param.bc = open_atm;
      }      
